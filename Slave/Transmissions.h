...
Transmission decode_response()
{
	Transmission transmission = {0, 0, 0, 0};
	transmission.options = PACKET_BUFFER[OPTIONS];

	transmission.current_position =	PACKET_BUFFER[CURRENT_UP] - 1 << 14
									| PACKET_BUFFER[CURRENT_MID] - 1 << 7
									| PACKET_BUFFER[CURRENT_LOW] - 1;

	transmission.desired_position =	(PACKET_BUFFER[DESIRED_UP] - 1 << 14)
									| (PACKET_BUFFER[DESIRED_MID] - 1 << 7)
									| PACKET_BUFFER[DESIRED_LOW] - 1;

	transmission.length =	(PACKET_BUFFER[LENGTH_UP] - 1 << 14)
...
// Takes location of packet array, the location of transmission (to prevent needing to copy).
// Sets current position and length in base 128 + 1 equivalent values.
void encode_transmission(Transmission& transmission)
{
	for(int x = 0; x < PACKET_LENGTH; x++) PACKET_BUFFER[x] = 1;  // reset PACKET_BUFFER

	PACKET_BUFFER[CURRENT_LOW] = (transmission.current_position & 0x7F) + 1;
	PACKET_BUFFER[CURRENT_MID] = (transmission.current_position >> 7 & 0x7F) + 1;
	PACKET_BUFFER[CURRENT_UP] = (transmission.current_position >> 14 & 0x7F) + 1;

	PACKET_BUFFER[LENGTH_LOW] = (transmission.length & 0x7F) + 1;
	PACKET_BUFFER[LENGTH_MID] = (transmission.length >> 7 & 0x7F) + 1;
	PACKET_BUFFER[LENGTH_UP] = (transmission.length >> 14 & 0x7F) + 1;
}
...
bool read_response_successfully_into_buffer()
{
...
{
	if(CLIENT.available() < string_length) return false;  // not != b/c we could want to pull additonal data after match
...
// Takes a C string to compare read data with, the length of said string.
// Iterates through C-string, checking that each byte matches.
// Returns NULL (no mismatch) if the string matches the buffer, ELSE the last seen character, if mismatch or 1 if read
// is unavailable.
...
{
	while(CLIENT.available() && buffer_mismatches_string("Content-Length: "));  // skip ahead while not at right part
...
uint64_t message_length(uint8_t max_digits)
{
	...
	byte* number_buffer = calloc(max_digits+1, sizeof(byte));  // zero out for arbitrary number between [0, 10^max_digits]
	// (get next three bytes OR stop at newline) if buffer available (should be, but double check :) )
