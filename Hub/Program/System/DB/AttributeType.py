...
class AttributeType:
	def __init__(self, attribute_name: str, types: Union[Any, list]):
		self._attribute_name: str = attribute_name;
		self._types: tuple = types if(isinstance(types, tuple)) else (types,);
...
class ObjectAttributeType(AttributeType):
	def __init__(self, obj: object, attribute_type: AttributeType):
		AttributeType.__init__(self, attribute_type._attribute_name, attribute_type._types)
	...
	def __str__(self) -> str:
		attribute_name: str = self._attribute_name;
		attribute_value: Any = self.value();
		value_type_str: str = type(attribute_value).__name__;
		allowed_type_str: str = " or ".join([type_instance.__name__ for type_instance in self._types]);
