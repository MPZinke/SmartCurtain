...
void loop()
{
	...
	if(event.event_id)
	{
		...
		if(details.curtain_length) 
		{
			...
			if(event_and_curtain_position_are_similar) ;  // do nothing with the motor
			else if(curtain_event_travels_full_span(event.event_position, details.curtain_length))
				reset_curtain_length(move_to_opposite_and_calibrate_curtain(details.curtain_direction));
			else if(position_not_reached_after_moving(&details, event.event_position) && auto_correct())
				assume_length_and_move_to_desired_position
...
void assume_length_and_move_to_desired_position(CURTAIN_DETAILS* details, long event_position)
{
	...
	// determine current position and pick number of steps from there
	// move other direction, number of steps, and assign other pin as stop pin
...
bool direction(int pin, bool DB_move_direction)
{
	pin == OPEN_PIN ^ DB_move_direction;
}
...
long needed_steps(int current_pin, long curtain_length, long event_position)
{
