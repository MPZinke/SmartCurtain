...
#define GPIO_H
	...
	bool curtain_reached_a_stop_button();
	bool direction(long, long, bool);
	void disable_motor();
	void enable_motor();
	long needed_steps(long, long);
	bool stop_button_reached(char);
	...
	bool stop_button_reached(char stop_button)
	{
	...
	int step_motor_to_location_or_end(bool move_direction, register long move_steps, register char opposing_stop_pin)
	{
		...
		while(0 < move_steps)
		{
			...
			delay(PULSE_WAIT);
			move_steps -= 3;
			if(!digitalRead(opposing_stop_pin)) return move_steps > 5 ? (int)move_steps : 0;
		}
